...
#define SERVEURNAME "172.18.41.144" // adresse IP de mon serveur
...
int main (  int argc, char** argv )
{
	...
	serveur_addr.sin_port = htons(30404);
	...
	send(to_server_socket,"Joueur1",7,0);
	memset(buffer, 0, sizeof(buffer));
	recv(to_server_socket,buffer,512, 0);
	printf("[client] %s [du serveur]\n", buffer);

	if(strcmp(buffer,"confirmation connection") == 0){
		send(to_server_socket,"ready",7,0);
		memset(buffer, 0, sizeof(buffer));

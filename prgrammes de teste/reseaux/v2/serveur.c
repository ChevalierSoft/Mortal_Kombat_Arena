...
int main ( void )
{
	...
	int quitter=0;
		...
		if(strncmp("joueur", buffer, 5)==0){
			printf("[client] %s [au serveur]\n",buffer);
			printf("[serveur] envoi de la réponse [au client] \n");

			sprintf(buffer,"Confirmation joueur");
			send(client_socket, buffer, 512, 0);
			printf("[du serveur] %s [client]\n", buffer);
			...
			if(strcmp("ready",buffer)==0){
				sprintf(buffer,"gg"); /* on envoie le premier jouer par le joueur p1 le nom du perso, ces coordo et oui*/
				send(client_socket, buffer, 512, 0);
			}
		}
	 	if(strncmp("QUITTER", buffer, 7) == 0) {
